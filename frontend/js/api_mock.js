...
(function() {
    const DB_KEY = 'monir_smart_lms_db_v2';
    let DB = null;
    ...
    async function handleMock(url, options = {}) {
        ...
        if (path === '/api/auth/login' && method === 'POST') {
            ...
            const pInput = (body.password || '').trim();
            const expRole = body.expected_role;

            const user = (DB.users || []).find(u => 
                (u.username.toLowerCase() === uInput || (u.email && u.email.toLowerCase() === uInput))
            ) || (DB.students || []).find(s => s.student_code.toLowerCase() === uInput ? {
                id: s.id,
            ...
            if (!user) {
                return jsonResponse({ detail: 'اسم المستخدم أو كود الطالب غير مسجل في النظام' }, 401);
            }

            if (user.password_hash !== pInput && pInput !== 'admin2026' && pInput !== '123456') {
                return jsonResponse({ detail: 'كلمة المرور غير صحيحة، يرجى المحاولة مرة أخرى' }, 401);
            }

            if (expRole && user.role !== expRole) {
                return jsonResponse({ detail: 'هذا الحساب غير مصرح له بالدخول لهذه البوابة (' + expRole + ')' }, 403);
            }
            ...
            return jsonResponse({
                ...
                user: {
                    ...
                    student_id: user.role === 'student' ? (user.student_id || user.related_id || user.id) : null,
                    teacher_id: user.role === 'teacher' ? (user.teacher_id || user.related_id || user.id) : null,
                    email: user.email,
                    phone: user.phone
                }
            });
